...
- **内容**（自 v0.11.3）：
  - 📋 **剪贴板历史**（clipboard）：后台常驻监听系统剪贴板（copypasta），任何应用复制都自动收录；点击条目即再复制供粘贴；可清空/移单
  - 🍅 **番茄钟**（pomodoro）：专注/休息循环，兼倒计时与秒表，专注/休息时长可在设置调整
  - 📈 **行情速览**（marketquotes）：汇率(frankfurter)与 BTC/ETH/SOL/BNB(CoinGecko) 小卡片，CNY/USD 一键折算
  - 📰 **资讯聚合**（news）：cnBeta/少数派/36氪/GitHub 等多个 RSS 源经 rss2json 代理，标题+相对时间，点击默认浏览器打开
  - 📶 **网络监控**（netmon）：系统各网卡实时上行/下行速率（sysinfo + managed 采样）
  - 🎂 **纪念日/倒数日**（anniversary）：记录重点日期（每年重复或一次性），显示还剩/已过多少天
  - 出厂即自带（加入 resources/plugins 打包）；主市场索引指向新增 6 插件的提交
- **发布流程**：release/v0.12.0 分支 → 版本号 0.11.3→0.12.0 → 合并 → 标签 v0.12.0 → CI 草稿 Release
## 2026-09-03（发布 v0.11.3：删除图标平滑消失动效 + 简化尺寸操作）✅

- **内容**（自 v0.11.2）：
  - ✨ **删除图标动效**：删除确认后图标平滑"微放大 → 缩小 + 淡出"消失（不再散块/彩块）
  - ✂️ **简化尺寸调整**：去掉 ⇲"设置尺寸"弹窗按钮，只保留 ⤡ 拖拽手柄调大小（一个入口即可）；已存尺寸不变
  - 🐛 布局修复关联：可逆的只读窗口适配（分辨率变化不保存、恢复还原布局)
- **发布流程**：release/v0.11.3 分支 → 版本号 0.11.2→0.11.3 → 合并回 main → 打标签 v0.11.3 → CI 自动构建并创建草稿 Release
## 2026-08-27（发布 v0.11.2：拖拽全面修复）✅

- **内容**（自 v0.11.1）：
  - 🐛 **修复待办等小组件无法拖动**：小组件内部按钮（列表项/勾选等）不再拦截拖拽按下事件（只忽略磁贴自身的操作按钮）；编辑模式下网格 touch-action: none，触屏拖动优先于小组件内部滚动
  - 🐛 **窗口变化自动适配**：分辨率/窗口缩放（含 pad ↔ 全屏切换）时自动把横向/纵向越界或重叠的图标移入当前窗口（不再需要紧凑重排、编辑模式拖拽始终可用）
  - 🐛 拖拽落点/改尺寸后自动修复残留重叠（多格重叠只交换第一个的问题）
- **发布流程**：release/v0.11.2 分支 → 版本号 0.11.1→0.11.2 → 合并回 main → 打标签 v0.11.2 → CI 自动构建并创建草稿 Release
## 2026-08-27（发布 v0.11.1：cmd 黑窗 + 图标重叠/超屏修复）✅

- **内容**（自 v0.11.0）：
  - 🐛 **修复打开应用/文件/文件夹时弹出黑色 cmd 窗口**（Windows `cmd /C start` 加 `CREATE_NO_WINDOW`，后台静默执行）
  - 🐛 **修复图标重叠**：拖拽落点只与第一个重叠单元交换、多格重叠残留 → 落点/调尺寸后自动把重叠图标移到同页空闲位（repairPageOverlaps）；启动时一次性修复历史遗留的重叠/超屏图标
  - 🐛 **图标不允许超出屏幕**：拖拽钳制在可视区内（拖不出屏幕下方）；隐藏纵向/横向滚动；分辨率变小导致的越界由显示层紧凑重排可逆处理（不保存）
- **发布流程**：release/v0.11.1 分支 → 版本号 0.11.0→0.11.1 → 合并回 main → 打标签 v0.11.1 → CI 自动构建并创建草稿 Release
## 2026-08-25（发布 v0.11.0：分辨率保持布局 + 图片图标 + 文件夹内启动修复）✅

- **内容**（自 v0.10.0）：
  - 🖥️ **分辨率/窗口变化不再打乱布局**：不再自动重排并持久化（旧行为会永久改序）；任一页图标越界时仅**显示层**按原顺序（行→列）逐页紧凑重排——**每页独立、页内滚动、不跨页不混排**，页1图标永远在页1；分辨率恢复后存储布局精确还原（不修改、不保存坐标）
  - 🖼️ **图片图标**（自定义图标弹窗"🖼 来自图片…"）：选任意图片作为图标，自动压缩到 ≤256px 存 data URL，圆角显示与应用图标同尺寸；优先级 图片 > 借用系统图标 > emoji；布局 schema 新增 iconImage 字段
  - 🐛 **修复**：文件夹内的图标（网页快捷方式/应用/文件）点击启动报 `cell not found`（launch_cell 只遍历顶层、不查文件夹 items）
- **发布流程**：release/v0.11.0 分支 → 版本号 0.10.0→0.11.0 → 合并回 main → 打标签 v0.11.0 → CI 自动构建并创建草稿 Release
## 2026-08-25（发布 v0.10.0：文件夹改造 + 统一图标标准 + 文件快捷方式 + 智能家居自定义）✅

- **内容**（自 v0.9.0）：
  - 📁 **文件夹改造**：大文件夹不再显示图标预览（只显示文件夹图标，尺寸与应用图标一致）；去掉"N 个图标"数量；打开/关闭过渡改为与应用窗口一致的淡入缩放；文件夹内点空白返回上一层
  - 📐 **统一图标设计标准**（iconStandard.ts）：emoji 图标字号=应用图标同尺寸；标签 13px/500 全局一致；圆角统一；网格间距滑块上限 12→30
  - 📄 **文件/文件夹快捷方式**（系统应用插件）：面板新增"选文件/选文件夹"，点击用默认程序/资源管理器打开；文件自动获取系统文件类型图标（docx/pdf/图片等）
  - 🏷️ **图标显示名称开关**：自定义图标弹窗"名称"右侧 👁 显示 / 🙈 隐藏（每图标独立）
  - 🏠 **智能家居自定义**（与系统图标同一套 📝 弹窗）：自定义名称 / emoji 图标 / 开状态颜色 / 隐藏名称，不再到实体选择界面改图标
  - 🐛 **修复**：文件夹尺寸重启后重置为 1×1（Rust 布局 schema 缺 folder.size，往返丢失）；网页快捷方式标题/图标抓取阻塞主线程导致冻结（改 async + spawn_blocking）
- **发布流程**：release/v0.10.0 分支 → 版本号 0.9.0→0.10.0 → 合并回 main → 打标签 v0.10.0 → CI 自动构建并创建草稿 Release
## 2026-02-13（发布 v0.9.0：拼音搜索 + 搜索过滤动画 + 全页搜索）✅

- **内容**（自 v0.8.0）：
  - 🔤 **拼音搜索**（`pinyin-pro`）：输入拼音（keting/kt）或拼音首字母即可命中中文名称（客厅灯/卧室灯等），覆盖顶部搜索、全局面板、文件夹搜索、智能家居实体名；拼音带缓存加速
  - 🎬 **搜索过滤动画**：搜索时命中的图标从**左上角依次紧凑排列**，带 0.28s 缓动过渡（过滤/变形效果）；清除搜索平滑归位
  - 🌐 **全页全局搜索**：搜索覆盖所有页面内容，命中项全部汇集到前面（不再只搜当前页）
  - 📦 新依赖：pinyin-pro
- **发布流程**：release/v0.9.0 分支 → 版本号 0.8.0→0.9.0 → 合并回 main → 打标签 v0.9.0 → CI 自动构建并创建草稿 Release
## 2026-02-13（发布 v0.8.0：插件搜索名接口 + 滚轮翻页 + 智能家居搜索）✅

- **内容**（自 v0.7.0）：
  - 🔍 **插件搜索名注册接口**（`registerSearchNames(pluginId, entries)`）：插件/小组件可注册额外可搜索名称；HomeAssistant 把全部实体友好名注册进搜索（"客厅灯/卧室灯"等任意实体可搜到）
  - 🕵️ **搜索双入口命中**：顶栏搜索框（网格过滤）+ 全局搜索面板（SearchPanel）均包含注册的智能家居名称，点击/回车自动定位并闪现对应小组件
  - ⌨️ **IME（输入法）搜索修复**：组词期间 `bind:value` 不提交 + 搜索过滤卸载 widget 清掉注册名 → 输入法搜不到；改为注册名粘性常驻 + 防空覆盖 + compositionend 强制取最终值
  - 🐛 **搜索时"连接中"闪烁**：HA 小组件重挂时从运行时缓存立即恢复状态，不再闪加载
  - 🖱️ **鼠标滚轮翻页**：复用滑动页切换动画（两段式滑出/滑入），400ms 防抖，可滚动小组件上不拦截
  - 📝 文档：主界面截图更新 + 新增英文 README
- **发布流程**：release/v0.8.0 分支 → 版本号 0.7.0→0.8.0 → 合并回 main → 打标签 v0.8.0 → CI 自动构建并创建草稿 Release
## 2026-02-13（发布 v0.7.0：插件提供商系统 + HomeAssistant 全面升级）🚀

- **内容**（自 v0.6.2）：
  - 🧩 **提供商系统（插件二级菜单）**：一个提供商可提供多个子插件（如 HomeAssistant → 💡灯/🔌开关/📊传感器）；AddMenu 二级菜单展开、提供商级共享配置（地址/令牌一处填写全子插件生效）；核心库支持 SubManifest 展平
  - 🏠 **HomeAssistant 智能家居全面升级**：
    - 设置菜单**自动拉取实体**勾选添加（新增 ha_entities 命令，按域过滤）
    - **MDI 图标选择器**（预设网格 + 自定义图标名，@mdi/js 内联 SVG，不依赖字体文件）
    - 实体**自定义显示名称**（空=恢复原名）
    - **大圆形状态图标**：开=实心琥珀色 FFC107 白图标，关=灰色；点击切换（乐观更新即时变色，不被轮询回翻）
  - 🎨 **统一图标设计标准**（iconStandard.ts）：所有插件图标同尺寸、居中、图标-文字间距一致
  - 🔲 **网格重构**：移动最小单位=网格大小（间距不干扰吸附）；增量式拖动（抓取位置不影响，拖一格动一格）；网格间距滑块（不影响图标大小）；调整尺寸改整格吸附
  - 🐛 **修复**：布局保存静默失败（拖拽调整尺寸产生小数尺寸导致 Rust 反序列化失败 → 布局永不落盘/重启还原）；WebView2 下图标状态更新残留
  - 📦 新依赖：@mdi/js（Material Design Icons 图标库）
- **发布流程**：release/v0.7.0 分支 → 版本号 0.6.2→0.7.0 → 合并回 main → 打标签 v0.7.0 → CI 自动构建并创建草稿 Release
## 2026-02-13（发布 v0.6.2：日历农历 + 待办字体优化）✅

- **内容**（自 v0.6.1）：
  - 📅 **日历农历**：新建农历转换库（经典查表法 1900-2100，干支年/月名/日名/闰月，单测 6/6）；日历组件每个日期下方显示农历（初一显月名、春节/中秋/端午/除夕等节日红字），头部显示农历干支年月
  - 🔤 待办组件字体整体调大（列表 12px、详情 14px 等，可读性优化）
- **发布流程**：release/v0.6.2 分支 → 版本号 0.6.1→0.6.2 → 合并回 main → 打标签 v0.6.2 → CI 自动构建并创建草稿 Release
## 2026-02-13（发布 v0.6.1：待办级联完成 + 提醒设置 + 视觉优化）✅

- **内容**（自 v0.6.0）：
  - ✅ 待办**级联完成**：全部子任务完成 → 父任务自动完成（逐级向上）；取消子任务父任务自动取消；父任务勾选级联后代
  - ⏰ **提醒设置**：截止日期+时间（预设时间胶囊 09:00~22:00 + 双列滚动自定义），提前量选项（不提醒/准时/5分/15分/30分/1小时/1天），到点系统通知
  - 🎨 视觉优化：圆形勾选按钮（主题色）、时间选择器随主题（color-scheme + 自定义控件）、层级仅靠缩进区分
  - ⚠️ 快捷键冲突提示：保留默认键位，注册失败时设置页显示警告（不影响启动）
  - 🔧 修复原生时间选择器白色背景、子任务层级缩进错位
- **发布流程**：release/v0.6.1 分支 → 版本号 0.6.0→0.6.1 → 合并回 main → 打标签 v0.6.1 → CI 自动构建并创建草稿 Release
## 2026-02-13（发布 v0.6.0：待办插件全面升级 + 系统集成修复）✅

- **内容**（自 v0.5.1）：
  - **✅ 待办插件 v2**：可视化日期时间选择器（月历网格 + 今天/明天/下周快捷 + 时间）；**多级子任务**（递归、展开折叠、子任务进度）；**到时提醒**（截止日期+时间，到点系统通知一次，改期重排）；详情弹层（文本/优先级/可视化时间/删除）；递归进度/排序/过滤/清空
  - **🔧 修复**：任务栏按钮残留（清除 WS_EX_APPWINDOW）；全屏时**勿扰（专注助手）图标闪动**（加入 FullScreenExclusionList，NSIS 安装钩子自动注册）
  - 恢复真全屏（覆盖整个屏幕）
- **发布流程**：release/v0.6.0 分支 → 版本号 0.5.1→0.6.0 → 合并回 main → 打标签 v0.6.0 → CI 自动构建三平台并创建草稿 Release
## 2026-02-13（发布 v0.5.1：滑动修复 + 市场索引更新）✅
...
- **内容**（自 v0.4.1）：
  - **🎵 音乐控制插件**（dev.homedesktop.music）：通过 Windows SMTC 显示当前播放（封面/标题/歌手/来源应用），上一曲/播放暂停/下一曲控制；跳曲后自动播放（针对 AmcfyMusic 等播放器状态失步的"延迟踢击"方案）；单行紧凑布局（2×1/3×1/5×1）
  - **🎤 歌词**：LRCLIB 免费接口获取同步歌词，逐行卡拉OK式高亮 + 自动居中滚动；精确匹配失败自动搜索兜底；请求 8s 超时 + 标题横跳防抖
  - **滑动切页**：鼠标左滑/右滑、触屏左右滑动切换页面（跟手阻尼 + 两段式滑入动画）
  - **画布溢出自动分页**：内容放不下自动放到下一页，杜绝滚动条；网格 overflow-y: hidden 硬性兜底
  - **添加图标体验**：自动跳到放置页；入场特效（缩放弹出，设置→特效 可开关）
  - **窗口显示/隐藏过渡**：淡出缩放 + 淡入（macOS 风格，设置→特效 可开关）
  - **自定义 exe 添加**：系统应用面板选择任意 exe，自动提取图标
  - 设置面板隐藏滚动条；全局 JS 错误捕获写入日志
- **修复**：addCell 漏 import 导致添加无反应；build.rs 复制 WebView2Loader.dll 触发 dev 无限重建；dev 残留 vite 占用 1420 端口
- **发布流程**：release/v0.5.0 分支 → 版本号 0.4.1→0.5.0 → 合并回 main → 打标签 v0.5.0 → CI 自动构建三平台并创建草稿 Release
## 2026-02-13（画布溢出自动分页：绝不出现滚动条）

- **需求**（用户提出）：内容放不下时不要再显示进度条（滚动条），应该自动放到下一页
- **实现**：
  - `fitCellsToCols` 适配后仍有单元超出视口行数 → 按位置顺序**自动移到下一页**空位（放不下继续往后，直到新建页），不再压缩重排
...
- **后续优化**：添加图标/新建文件夹时若被自动放到下一页，**应用自动切换到该页**（用户能立刻看到图标加到了哪里，符合手机桌面习惯）；`addCell` 返回实际放置页码，测试补充"当前页放满→下一页并返回页码"
- **后续优化**：添加图标**入场特效**——新图标从 15% 缩放弹出（弹性缓动，1.6s 后自动清除）；设置 → 特效 面板可单独开关（persisted 于 config.json appearance.effects.iconAdd）
- **调整**（用户反馈）：去掉光环扩散，只保留缩放弹出；特效开关放设置里单独的区域
- **后续优化**：**窗口显示/隐藏过渡特效**（macOS 风格）——隐藏时淡出 + 轻微缩放（200ms），显示时淡入；托盘/Alt+Space 的隐藏改为发事件给前端播完动画再真正隐藏；设置 → 特效 可单独开关（effects.windowAnim）
- **调整**（用户反馈）：设置面板隐藏竖向滚动条（scrollbar-width: none + ::-webkit-scrollbar display:none，滚轮仍可滚动）
- **后续优化**：**鼠标左滑/右滑、触屏左右滑动切换页面**（手机桌面习惯）——按住左右拖动手感跟手（阻尼跟随），松手达阈值两段式滑出/滑入动画切页，未达回弹；仅正常模式生效（编辑模式拖拽图标不受影响），滑动后抑制图标点击/空白隐藏误触
- **后续优化**：系统应用面板新增"**选择自定义 exe 位置…**"——文件对话框选任意 .exe，自动取文件名并提取应用图标加入桌面（borrow 模式同样可用）
- **新插件：🎵 音乐控制**（dev.homedesktop.music）——通过 Windows SMTC（系统媒体传输控制）显示当前播放的音乐（封面/标题/歌手/来源应用），提供**上一曲 / 播放暂停 / 下一曲**按钮；封面经 SMTC 缩略图读取为 data URL（≤2MB，PNG/JPEG 魔数识别）；专用 MTA 工作线程 + 通道（避免阻塞主线程）；非 Windows 平台命令返回"不支持"提示；顺带修复 build.rs 复制 WebView2Loader.dll 触发 tauri dev 无限重建循环（内容相同跳过）
- **修复**（用户反馈）：点击下一曲/上一曲后不自动播放——AmcfyMusic 等播放器 SMTC 跳曲后状态与音频失步（报 Playing 但无声）。对策：跳曲成功后等 1.5s（播放器加载曲目），再做"暂停→播放"踢击强制重启音频引擎（最多 3 轮，实测 1 轮即恢复自动播放）；保留状态变化追踪日志便于排查播放器兼容性
- **新功能：🎤 歌词**（用户提出）——音乐小组件新增歌词按钮，打开全屏歌词面板（LRCLIB 免费接口，无需密钥）：SMTC timeline 提供播放进度（MediaInfo 增加 position/duration），LRC 逐行解析，**随播放逐行高亮 + 自动居中滚动**（卡拉OK式）；精确匹配失败自动搜索取时长最接近版本；纯音乐/未找到有对应提示
## 2026-02-13（发布 v0.4.1：网页图标缓存与稳定性修复）✅

- **内容**（自 v0.4.0）：
  - **网页图标磁盘缓存**：按网址抓取一次并缓存（icons/web/），之后直接读缓存不再联网；图标尊重各实例已保存的网址（此前回退默认百度）
  - **崩溃修复**：抓取中文页面时 UTF-8 截断越界 panic；图标大小上限 300KB → 4MB（大图标站点可显示）
- **发布流程**：release/v0.4.1 分支 → 版本号 0.4.0→0.4.1 → 合并回 main → 打标签 v0.4.1 → CI 自动构建并创建草稿 Release
## 2026-02-13（发布 v0.4.0：网页快捷方式 + 自动隐藏）✅

- **内容**（自 v0.3.0）：
  - **网页快捷方式插件**（🌐 web-shortcut）：自定义网址、点击用默认浏览器打开；自动获取**网站图标**（页面声明 icon → favicon.ico/png，自签证书站点也支持，data URL 方案）与**网站标题**（Rust 抓取 + 免证书校验）
  - **自动隐藏**：启动外部应用/网页后隐藏本应用；点击页面空白区域隐藏（Alt+Space 唤出）
  - 在线市场 11 个插件（jsDelivr 分发）
  - 动作 {设置键} 占位符能力（插件可配置动作）
- **发布流程**：release/v0.4.0 分支 → 版本号 0.3.0→0.4.0 → 合并回 main → 打标签 v0.4.0 → CI 自动构建三平台并创建草稿 Release
## 2026-02-13（网页插件：网站图标 + 自动获取网站标题）✅ 用户验收通过

- **需求**（用户提出）：网页快捷方式可以获取网站图标、网站标题
- **实现**：
  - **网站图标**：新组件 FaviconIcon —— 读取实例 url → 显示网站 favicon（兜底链 /favicon.ico → /favicon.png → emoji，不依赖第三方服务）；含 url 设置项的插件图标自动生效
  - **网站标题**：Rust 命令 web_fetch_title（ureq 拉取 HTML，提取 &lt;title&gt;，实体解码/去空白/限长 60）→ 添加网页图标或修改网址时自动把图标名设为网站标题
- **验证**：cargo check 通过；svelte-check 0/0；vitest 83/83；GitHub/百度标题提取实测正常
- **后续修复**（用户实测反馈迭代）：① 无 /favicon.ico 的站点 → 解析页面 link rel=icon 声明图标；② 自签证书站点（OpenWrt 路由器）→ Rust 免证书校验抓取 + 转 data URL（WebView 加载远程图不跳过证书校验）；③ FaviconIcon 响应式（改网址自动重抓图标）
...
- **内容**（自 v0.2.1）：
  - **自由摆放（v3）**：主页面 + 文件夹内拖拽任意定位，吸附网格、占用即交换、不自动重排；旧布局自动迁移分配坐标
  - **仅全屏模式**：启动即全屏（窗口 fullscreen + 启动兜底），移除窗口模式（⛶ 切换/Esc 退出）
  - **网格自适应列数**：按窗口宽度计算列数，画布恰好铺满；越界/重叠单元自动移回空位（含高度边界压缩，消除滚动条）；移除页面内 ＋ 占位，统一顶栏 ＋
  - 多项修复：全屏启动卡死（setFullscreen 不阻塞）、添加按实际尺寸找位、fit 后自动持久化
- **发布流程**：release/v0.3.0 分支 → 版本号 0.2.1→0.3.0 → 合并回 main → 打标签 v0.3.0 → CI 自动构建三平台并创建草稿 Release
## 2026-02-13（仅全屏模式 + 网格自适应列数）⏳ 待验收

- **需求**（用户提出）：① 图标被窗口截掉一半 → 网格列数按窗口大小自适应（能放下的最小列数）；② 直接按全屏开发，不做窗口模式
- **实现**：窗口 fullscreen: true 启动即全屏（移除 ⛶ 切换/Esc 退出/全屏记忆）；Grid 按窗口宽度动态计算列数（画布恰好铺满窗口），activePageCols 同步给 stores；窄窗口保留横向滚动兜底
